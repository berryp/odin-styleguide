...
load_roster :: proc(path: string, allocator := context.allocator) -> (roster: Roster, err: Error) {
	data, read_err := os.read_entire_file(path, allocator)
	if read_err != nil {
		// Convert the low-level os error into a rich, specific error value.
...
save_roster :: proc(path: string, roster: Roster, allocator := context.allocator) -> Error {
